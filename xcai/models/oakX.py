...
# %% auto 0
__all__ = ['Encoder', 'OAK000', 'OAK001', 'OAK002', 'OAK003', 'OAK004', 'OAK005Encoder', 'OAK005', 'OAK006Encoder', 'OAK006',
           'OAK007Encoder', 'OAK007']
...
# %% ../../nbs/26_models.oakX.ipynb 90
class OAK006Encoder(Encoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(config, num_metadata=num_metadata, **kwargs)
        self.pretrained_meta_embeddings = nn.Embedding(num_metadata, metadata_dim)
        self.projector = nn.Linear(metadata_dim, config.dim)

    def init_embedding_projector(self):
    ...
    def fuse_meta_into_embeddings(self, data_repr:torch.Tensor, data_mask:torch.Tensor, meta_kwargs:Dict):
        ...
        for m_key, m_args in meta_kwargs.items():
            ...
            if len(idx):
                ...
                m_repr = F.normalize(self.projector(self.pretrained_meta_embeddings(m_idx)), dim=1)
...
class OAK006(OAK000, DistilBertPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder = OAK006Encoder(config, num_metadata=num_metadata, metadata_dim=metadata_dim, resize_length=resize_length)
        self.post_init(); self.remap_post_init(); self.init_retrieval_head(); self.init_cross_head()

    def _get_encoder_meta_kwargs(self, feat:str, prefix:str, **kwargs):
        return Parameters.from_feat_meta_aug_prefix(feat, prefix, **kwargs)
        
    def remap_post_init(self):
        self.distilbert = self.encoder.distilbert
        

# %% ../../nbs/26_models.oakX.ipynb 105
class OAK007Encoder(Encoder):
    ...
    def fuse_meta_into_embeddings(self, data_repr:torch.Tensor, data_mask:torch.Tensor, meta_kwargs:Dict):
        ...
        for m_key, m_args in meta_kwargs.items():
            ...
            if len(idx):
                m_idx,m_repr,m_repr_mask = self.resize(m_args['idx'], m_args['meta_repr'], m_args['data2ptr'][idx])
                m_repr = F.normalize(m_repr + self.projector(self.pretrained_meta_embeddings(m_idx)), dim=1)
...
# %% ../../nbs/26_models.oakX.ipynb 106
class OAK007(OAK000, DistilBertPreTrainedModel):
    use_generation,use_representation = False,True
    _tied_weights_keys = ["encoder.distilbert"]
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(config, num_metadata=num_metadata, **kwargs)
        self.encoder = OAK007Encoder(config, num_metadata=num_metadata, metadata_dim=metadata_dim, resize_length=resize_length)
        self.post_init(); self.remap_post_init(); self.init_retrieval_head(); self.init_cross_head()
