...
# %% auto 0
__all__ = ['UPMAConfig', 'get_memory_module', 'get_loss_function', 'align_tensor', 'alignment_mask', 'verify_tied_weights',
           'UPMAEncoderOutput', 'UPMAModelOutput', 'FFN', 'UPMAEmbeddingMemory', 'UPMAEncoderMemory', 'Parameters',
           'UPMAModel', 'UPMAEncoder', 'UPA000']
...
class UPMAConfig(DistilBertConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        store_attr('n_memory_layers,tie_memory_encoder_weights,exclude_module_from_tying')
...
# %% ../../nbs/41_models.upma.ipynb 21
def verify_tied_weights(targ:nn.Module, src:nn.Module, name:str, exclude_module:Optional[str]=None):
    sd_src, sd_targ = get_attr(src, name).state_dict(), get_attr(targ, name).state_dict()
    keys = (
        sd_targ.keys() if exclude_module is None else 
        [k for k in sd_targ.keys() if not re.match(exclude_module, k)]
    )
    return np.all([sd_src[k].data_ptr() == sd_targ[k].data_ptr() for k in keys])
    

# %% ../../nbs/41_models.upma.ipynb 27
@dataclass
        ...
        for module, injection_layer in zip(targ_model.memory_modules, config.memory_injection_layers):
            if config.initialize_memory_embeddings_from_injection_layer_mean and isinstance(module, UPMAEmbeddingMemory):
                ...
                for module_name in ['embeddings', 'layer']:
                    ...
                    assert verify_tied_weights(module, targ_model, module_name, config.exclude_module_from_tying)
