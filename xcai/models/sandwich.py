...
class SAW000(nn.Module):
    ...
    def compute_meta_loss(self, data_o, lbl2data_o, **kwargs):
        ...
        if meta_kwargs is not None and prefix in meta_kwargs and len(meta_kwargs[prefix]['idx']):
...
class Encoder004(BaseEncoder):
    ...
    def enrich_query_representation(self, data_repr:torch.Tensor, data_meta_repr:torch.Tensor):
        fused_repr = self.combiner_head(torch.cat([data_repr, data_meta_repr], dim=-1))
