...
# %% auto 0
__all__ = ['MainXCData', 'NegXCData', 'MetaXCData', 'BaseXCDataset', 'MainXCDataset', 'MetaXCDataset', 'Operations',
           'MetaXCDatasets', 'XCDataset', 'XCCollator', 'BaseXCDataBlock', 'XCDataBlock']
...
class NegXCData:
    ...
    def from_file(cls, data_neg:str, neg_info:Optional[str]=None, main_max_lbl_sequence_length:Optional[int]=None,
        ...
        return {
            ...
            'neg_info': (
                Info.from_txt(neg_info, max_sequence_length=main_max_lbl_sequence_length, prompt_func=lbl_prompt_func, **kwargs) 
                if isinstance(neg_info, str) else neg_info
            ),
        }
...
class MainXCDataset(BaseXCDataset):
    ...
    def from_file(cls, **kwargs):
        data_args_name = list(inspect.signature(MainXCData.from_file).parameters)
        negs_args_name = list(inspect.signature(NegXCData.from_file).parameters)

        shared_args_name = set(data_args_name).intersection(negs_args_name)
        shared_args = {o: kwargs.pop(o) for o in shared_args_name if o in kwargs}
        
        data_args = {o: kwargs.pop(o) for o in data_args_name if o in kwargs}
        negs_args = {o: kwargs.pop(o) for o in negs_args_name if o in kwargs}
    ...
    def _verify_inputs(cls):
        ...
        if cls.data_neg is not None:
            ...
            if cls.neg_info is not None:
                n_neg = cls._verify_info(cls.neg_info)
                if n_neg != cls.n_neg:
                    raise ValueError(f'`neg_info`({n_neg}) should have same number of labels as `data_neg`({cls.n_neg})')
    ...
    def _store_scores(self):
        ...
        if self.data_lbl is not None:
            self.data_lbl_scores = [o.data.tolist() for o in get_matrix(self.data_lbl)]
            
        if self.data_neg is not None: 
            self.data_neg_scores = [o.data.tolist() for o in get_matrix(self.data_neg)]
    ...
    def _getitems(self, idxs:List):
        ...
        kwargs['data_neg'] = self.data_neg[idxs] if self.data_neg is not None else None
        return type(self)(**kwargs)
...
def __getitem__(cls:MainXCDataset, idx:int):
    ...
    if cls.data_neg is not None:
        prefix = 'neg2data'
        x[f'{prefix}_idx'] = cls.curr_data_neg[idx]
        if cls.return_scores: x[f'{prefix}_scores'] = cls.data_neg_scores[idx]
        if cls.n_neg_samples:
            rnd_idx = np.random.permutation(len(x[f'{prefix}_idx']))[:cls.n_neg_samples]
            x[f'{prefix}_idx'] = [x[f'{prefix}_idx'][i] for i in rnd_idx]
            if cls.return_scores: x[f'{prefix}_scores'] = [x[f'{prefix}_scores'][i] for i in rnd_idx]
        if cls.neg_info is not None:
            x.update({f'{prefix}_{k}':[v[i] for i in x[f'{prefix}_idx']] for k,v in cls.neg_info.items() if k in cls.lbl_info_keys})
    return x
...
class MetaXCDataset(BaseXCDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.curr_data_meta = self.curr_lbl_meta = self.curr_neg_meta = None
        self.data_meta_scores = self.lbl_meta_scores = self.neg_meta_scores = None
    ...
    def _verify_inputs(cls):
        ...
        if cls.neg_meta is not None:
            cls.n_neg = cls.neg_meta.shape[0]
            if cls.neg_meta.shape[1] != cls.n_meta:
                raise ValueError(f'`neg_meta`({cls.neg_meta.shape[1]}) should have same number of columns as `data_meta`({cls.n_meta}).')
    ...
    def _store_indices(self):
        ...
        if self.neg_meta is not None: self.curr_neg_meta = [o.indices.tolist() for o in self.neg_meta]
    ...
    def _store_scores(self):
        ...
        self.neg_meta_scores = get_scores(self.neg_meta, self.use_meta_distribution)
    ...
    def update_meta_matrix(self, data_meta:sparse.csr_matrix, lbl_meta:Optional[sparse.csr_matrix]=None, 
                           neg_meta:Optional[sparse.csr_matrix]=None):
        self.data_meta, self.lbl_meta, self.neg_meta = data_meta, lbl_meta, neg_meta
    ...
    def _sample_meta_items(self, idxs:List):
        ...
        kwargs['neg_meta'] = None if self.neg_meta is None else self.neg_meta[:, idxs]
        return type(self)(**kwargs)
    ...
    @dispatch
    def get_neg_meta(self, idx:int):
        if self.curr_neg_meta is None: return {}
        prefix = f'{self.prefix}2neg2data'
        x = {f'{prefix}_idx': self.curr_neg_meta[idx]}
        if self.return_scores: x[f'{prefix}_scores'] = self.neg_meta_scores[idx]
        if self.n_neg_meta_samples:
            rnd_idx = np.random.permutation(len(x[f'{prefix}_idx']))[:self.n_neg_meta_samples]
            x[f'{prefix}_idx'] = [x[f'{prefix}_idx'][i] for i in rnd_idx]
            if self.return_scores: x[f'{prefix}_scores'] = [x[f'{prefix}_scores'][i] for i in rnd_idx]
        if self.meta_info is not None:
            x.update({f'{prefix}_{k}':[v[i] for i in x[f'{prefix}_idx']] for k,v in self.meta_info.items() if k in self.meta_info_keys})
    ...
    def get_neg_meta(self, idxs:List):
        if self.curr_neg_meta is None: return {}
        prefix = f'{self.prefix}2neg2data'
        x = {f'{prefix}_idx': [self.curr_neg_meta[idx] for idx in idxs]}
        if self.return_scores: x[f'{prefix}_scores'] = [self.neg_meta_scores[idx] for idx in idxs]
        if self.n_neg_meta_samples:
            rnd_idxs = [np.random.permutation(len(o))[:self.n_neg_meta_samples] for o in x[f'{prefix}_idx']]
            x[f'{prefix}_idx'] = [[o[i] for i in idx] for o,idx in zip(x[f'{prefix}_idx'],rnd_idxs)]
            if self.return_scores: x[f'{prefix}_scores'] = [[o[i] for i in idx] for o,idx in zip(x[f'{prefix}_scores'],rnd_idxs)]
        if self.meta_info is not None:
            x.update({f'{prefix}_{k}':[[v[i] for i in o] for o in x[f'{prefix}_idx']] for k,v in self.meta_info.items() if k in self.meta_info_keys})
    ...
    def shape(self): return (self.n_data, self.n_lbl, self.n_neg, self.n_meta)
...
class Operations:
    ...
    def combined_lbl_and_meta(dset, meta_name:str, pad_token:Optional[int]=0, p_data:Optional[float]=0.5, **kwargs):
        ...
        data_dset = type(dset.data)(data_info, sp.hstack([data_lbl, data_meta]), combined_info, 
                                    dset.data.data_lbl_filterer, **args)
...
class XCDataset(BaseXCDataset):
    ...
    def _verify_inputs(self):
        self.n_data, self.n_lbl, self.n_neg, self.n_meta = self.data.n_data, self.data.n_lbl, self.data.n_neg, len(self.meta)
        for meta in self.meta.values():
            ...
            if self.n_neg is not None and meta.n_neg is not None and meta.n_neg != self.n_neg:
                raise ValueError(f'`meta`({meta.n_neg}) and `data`({self.n_neg}) should have the same number of negatives.')
    ...
    def _get_main_dataset(self, data_info:Dict, data_lbl:Optional[sp.csr_matrix]=None, lbl_info:Optional[Dict]=None, 
        ...
        dset = self.data._get_dataset(data_info, data_lbl=data_lbl, lbl_info=lbl_info, data_lbl_filterer=data_lbl_filterer, **kwargs)
    ...
    def meta_dset(self, meta_name, **kwargs):
        ...
        _keys_to_ignore = ["data_info", "data_lbl", "lbl_info", "data_lbl_filterer", "curr_data_lbl", "data_lbl_scores", 
                           "neg_info", "data_neg", "curr_data_neg", "data_neg_scores"]
    ...
    def __getitem__(self, idx:int):
        ...
        if self.n_meta:
            for m in self.meta.values():
                ...
                if self.n_neg: x.update(m.get_neg_meta(x['neg2data_idx']))
        return x
